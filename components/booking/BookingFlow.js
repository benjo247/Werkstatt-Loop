...
import { Wrench, Snowflake, Disc3, Search, Sparkles, ShieldCheck, ChevronRight, ChevronLeft, Check, Camera, Phone } from 'lucide-react';
...
export default function BookingFlow({ workshop, embed = false }) {
  ...
  // postMessage für Iframe-Höhe + Erfolg-Event
  useEffect(() => {
    if (!embed) return;
    function sendHeight() {
      try {
        window.parent?.postMessage({
          source: 'werkstattloop',
          type: 'resize',
    ...
    return () => { clearInterval(interval); window.removeEventListener('resize', sendHeight); };
  }, [embed, step]);

  useEffect(() => {
    if (!embed || step !== 5) return;
    try {
      window.parent?.postMessage({
        source: 'werkstattloop',
  ...
  async function handleFileUpload(e) {
    ...
    try {
      const res = await fetch('/api/public/ocr/vehicle-registration', { method: 'POST', body: formData });
  ...
  return (
    <div className={embed ? 'bg-slate-50' : 'min-h-screen bg-slate-50'} style={{ '--primary': primary }}>
      {!embed && (
      ...
      <main className={`${embed ? 'p-4 md:p-6' : 'max-w-3xl mx-auto px-5 py-8'}`}>
          ...
          {['Leistung', 'Schein', 'Termin', 'Daten', 'Fertig'].map((label, i) => {
            ...
            return (
              <div key={n} className="flex flex-col items-center gap-1.5 flex-1 first:items-start last:items-end">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${active ? 'text-white' : 'bg-slate-100 text-slate-400'}`} style={active ? { background: primary } : {}}>
                  {done ? <Check className="w-3 h-3" /> : n}
                </div>
                <span className={`text-[9px] uppercase tracking-wider font-bold ${active ? 'text-slate-900' : 'text-slate-400'}`}>
                  {label}
                </span>
              </div>
            );
        ...
        <div className="bg-white rounded-2xl p-5 md:p-7 shadow-sm border border-slate-200">

          {step === 1 && (
            <div>
              <h2 className="text-xl md:text-2xl font-archivo font-black mb-1">Welche Leistung?</h2>
              <p className="text-sm text-slate-600 mb-5 font-medium">Wähle den Service, den du brauchst.</p>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {SERVICES.map(s => {
                  const Icon = s.icon;
                  return (
                    <button key={s.id} onClick={() => { setService(s); setStep(2); }}
                      className="text-left p-4 rounded-xl border-2 border-slate-200 hover:border-slate-300 bg-white transition">
          ...
          {step === 2 && (
            <div>
              <h2 className="text-xl md:text-2xl font-archivo font-black mb-1">Fahrzeugschein hochladen?</h2>
              <p className="text-sm text-slate-600 mb-5 font-medium">
                Mit Foto sind die Daten in 5 Sek vorausgefüllt. Oder du tippst sie selbst ein — beides geht.
              </p>

              <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 mb-5">
                <p className="text-xs font-bold text-orange-900 uppercase tracking-wider mb-3">Datenschutz</p>
                <label className="flex items-start gap-3 cursor-pointer mb-3">
                  ...
                  <span className="text-xs text-slate-700 font-medium leading-relaxed">
                    Ich willige ein, dass das Foto zur Daten-Extraktion an <strong>Anthropic PBC (USA, EU-US Data Privacy Framework)</strong> übertragen wird.
                  </span>
                </label>
                <label className="flex items-start gap-3 cursor-pointer">
                  ...
                  <span className="text-xs text-slate-700 font-medium leading-relaxed">
                    Ich willige ein, dass das Foto in der Cloud (Vercel Blob, Frankfurt, max. 30 Tage nach Termin) für die Werkstatt gespeichert wird.
                  </span>
                </label>
              </div>
              ...
              <label className={`block border-2 border-dashed rounded-xl p-6 md:p-8 text-center cursor-pointer transition ${
                ...
                {uploading ? (
                  <>
                    <div className="w-10 h-10 mx-auto mb-3 border-4 border-orange-500 border-t-transparent rounded-full animate-spin"></div>
                    <p className="font-bold text-slate-900">Daten werden ausgelesen...</p>
                    <p className="text-xs text-slate-500 mt-1">~5 Sekunden</p>
                  </>
                ) : (
                  <>
                    <Camera className="w-10 h-10 mx-auto mb-3 text-slate-400" />
                    <p className="font-bold text-slate-900">Foto vom Fahrzeugschein</p>
                    <p className="text-xs text-slate-500 mt-1">Foto machen oder hochladen · max. 10 MB</p>
                  </>
                )}
              </label>
              ...
              <div className="flex justify-between items-center pt-4 mt-4 border-t border-slate-200">
                ...
                <button onClick={() => setStep(3)} className="text-sm font-bold text-orange-600 hover:text-orange-700">
                  Lieber manuell →
                </button>
              </div>
            </div>
          ...
          {step === 4 && (
            <div>
              <h2 className="text-xl md:text-2xl font-archivo font-black mb-1">Deine Daten</h2>
              <p className="text-sm text-slate-600 mb-5 font-medium">
                {registrationData ? 'Wir haben den Schein gelesen — bitte Name + Kontakt ergänzen.' : 'Damit wir dich erreichen können.'}
              </p>
              ...
              <div className="flex justify-between items-center pt-4 mt-4 border-t border-slate-200">
                ...
                <button onClick={submitBooking} disabled={!canSubmit || submitting} className="px-5 py-2.5 rounded-full font-bold text-sm flex items-center gap-1.5 text-white disabled:opacity-30" style={{ background: primary }}>
                  {submitting ? 'Sende...' : 'Termin anfragen'} <Check className="w-4 h-4" />
                </button>
              </div>
            </div>
          )}
          ...
          {step === 5 && (
            <div className="text-center py-6">
              ...
              <h2 className="text-xl md:text-2xl font-archivo font-black mb-2">Termin angefragt!</h2>
              <p className="text-sm text-slate-600 max-w-md mx-auto mb-5 font-medium">
                Die Werkstatt bestätigt deinen Termin per {form.email ? 'E-Mail' : 'SMS'} — meist innerhalb weniger Stunden.
              </p>
              <div className="bg-slate-900 text-white rounded-2xl p-5 max-w-sm mx-auto text-left">
...
function DateTimeStep({ date, setDate, time, setTime, onBack, onNext, service, primary }) {
  ...
  return (
    <div>
      <h2 className="text-xl md:text-2xl font-archivo font-black mb-1">Wann passt es dir?</h2>
      <p className="text-sm text-slate-600 mb-5 font-medium">Für <strong>{service?.name}</strong> · ca. {service?.duration}</p>
